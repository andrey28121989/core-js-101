...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    const result = str[i];
    if (str.lastIndexOf(result) === str.indexOf(result)) {
      return result;
    }
  }
  return null;
...
function getDigitalRoot(num) {
  let result = num.toString().split('').reduce((prev, curr) => prev + +curr, 0);
  while (result > 9) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.includes(str[i])) {
      ...
      if (pairBrackets[str[i]] === arrStack[arrStack.length - 1]) {
        arrStack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  let result = '';
  const chars = pathes.map((elem) => elem.split('/'));
  for (let i = 0; i < chars[0].length; i += 1) {
    if (chars.every((item) => item[i] === chars[0][i])) {
      result = result.concat(chars[0][i], '/');
    }
  }
  return result;
}
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[0][2] === position[1][1] && position[0][2] === position[2][0] && position[0][2]) {
    return position[0][2];
  }
